...
class RFBClient(Protocol):  # type: ignore[misc]
    ...
    def _handleRectangle(self, block: bytes) -> None:
        ...
        if self.rectangles:
            ...
            elif encoding == Encoding.PSEUDO_POINTER_POS:
                # The position rides in the rectangle header (x, y); there is
                # no payload, and moving the pointer paints nothing, so it is
                # not counted as a framebuffer rectangle.
                del self.rectanglePos[-1]
                self._handleDecodePointerPos(x, y)
    ...
    # --- Pseudo Pointer Position Encoding
    def _handleDecodePointerPos(self, x: int, y: int) -> None:
    ...
    def updateRectangle(
        ...
        :param pixel_format: the pixel format of ``data``.
        """
